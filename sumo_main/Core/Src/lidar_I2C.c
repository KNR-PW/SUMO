...
void Lidar_initALL()
{
	...
	//inizialization of Lidar 2
	HAL_GPIO_WritePin(XSHUT_2_GPIO_Port,XSHUT_2_Pin, 1);
	HAL_Delay(100);
	//change back address to default to be able to initialize second lidar
	*LidarAddr = 0b01010010;
	...
	//inizialization of Lidar 3
	HAL_GPIO_WritePin(XSHUT_3_GPIO_Port,XSHUT_3_Pin, 1);
	HAL_Delay(100);
	//change back address to default to be able to initialize third lidar
	*LidarAddr = 0b01010010;
	HAL_Delay(10);
	Lidar_init();
...
void Lidar_init()
{
	...
	if(!initVL53L0X(1, &hi2c1))
	{
		...
		setVcselPulsePeriod(VcselPeriodFinalRange, 14); //was 14			//docunemtation values: 8-14
		uint8_t b_timingBudgetChanged = setMeasurementTimingBudget(3000 * 1000UL);
