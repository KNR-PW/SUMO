...
//SUMO KNR part form here

//pointer that enables changing the address to lidar whitch we communicate currently
//the pointer is used because the pointing variable (g_i2cAddr) must be possible to change in cerain file (VL53L0X.c)
//from all places in the code. Using this weird structure - volatile pointer - enables that
extern volatile uint8_t * LidarAddr;

//this is the I2C that we use to communicate with lidar sensors
extern I2C_HandleTypeDef hi2c1;;

//initialize all I2C lidars
void Lidar_initALL();

//initilize one lidar module (the address must be set before manually),
//after basic initialization changes some parameters of sensor to be able to correcly read and send data
//in general the response time is extended
void Lidar_init();

//returns distanse from lidar
//this functions are almost the same, have only diffirend lidar address
uint16_t readLidar1();
uint16_t readLidar2();
uint16_t readLidar3();
